...
export default class BridgeLoginSubmitUserInput extends Command {
  static override summary = 'Submit user input for a bridge login step'
  ...
  static override flags = {
    ...
    body: Flags.string({ description: 'JSON object containing field IDs and string values', required: true }),
    debug: Flags.boolean({ default: false }),
    json: Flags.boolean({ default: false, description: 'Print JSON' }),
  }

  async run(): Promise<void> {
    const { args, flags } = await this.parse(BridgeLoginSubmitUserInput)
    const client = await createClient(flags)
    const body = JSON.parse(flags.body) as Record<string, string>
    const step = await client.matrix.bridges.auth.submitUserInput(args.stepID, {
